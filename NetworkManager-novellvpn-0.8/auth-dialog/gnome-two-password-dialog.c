...
gnome_two_password_dialog_get_remember (GnomeTwoPasswordDialog *password_dialog)
{
	...
	session = gtk_toggle_button_get_active (
			GTK_TOGGLE_BUTTON (password_dialog->details->remember_session_button));
	forever = gtk_toggle_button_get_active (
			GTK_TOGGLE_BUTTON (password_dialog->details->remember_forever_button));

	// When set the session and forever in same time, select forever in priority
	if (forever) {
		...
		gboolean editable)
{
	...
	gtk_widget_set_sensitive (GTK_WIDGET (password_dialog->details->password_entry_secondary), editable);
	gtk_editable_set_editable ((GtkEditable*)password_dialog->details->password_entry_secondary, editable);
}
